            ...
            <body>
                ...
                <table border="1">
                    ...
                    <xsl:for-each select="/xmi:XMI/refsem:OntologyConcept">
                        ...
                        <xsl:variable name="mention_elem"
                            select="//textsem:EntityMention[contains(@ontologyConceptArr, $ontologyId)]"/>
                        <xsl:variable name="concept_elem"
                            select="//types:Concept[@originalEntityExternalId = $mention_elem/@xmi:id]"/>

                        <xsl:variable name="polarity">
                            <xsl:choose>
                                <xsl:when test="$mention_elem/@polarity">
                                    <xsl:value-of select="$mention_elem/@polarity"/>
                                </xsl:when>
                        ...
                        <xsl:variable name="score">
                            <xsl:choose>
                                <xsl:when test="xs:decimal(@score) gt 0">
                        ...
                        <tr>
                            ...
                            <td>
                                <xsl:value-of select="$concept_elem/@conceptText"/>
                            </td>
                            <td>
                            ...
                            <td>
                                <xsl:value-of
                                    select="format-number(xs:decimal($score) * xs:decimal($polarity), '0.0')"
                                />
                            </td>

                            <td>
                                <xsl:value-of select="@disambiguated"/>
                            </td>
                            <td>
                                <xsl:value-of select="$mention_elem/@subject"/>
                            </td>
                            <td>
                                <xsl:value-of
                                    select="
                                        substring(//cas:Sofa/@sofaString,
                                        $mention_elem/@begin,
                                        xs:integer($mention_elem/@end) - xs:integer($mention_elem/@begin) + 1)"
                                />
                            </td>
                        </tr>
                    </xsl:for-each>
